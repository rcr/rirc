...
#define TEST_SEND_BUF_MAX 4
#define TEST_SEND_BUF send_buf[(send_buf_i - 1) % TEST_SEND_BUF_MAX]

// TODO: this works, but it's brittle in cases where the test fails
static int send_buf_i;
static int send_buf_n;
static char chan_buf[1024];
static char line_buf[1024];
static char send_buf[TEST_SEND_BUF_MAX][1024];
...
test_ircv3_CAP(void)
{
	...
	assert_strcmp(line_buf, "CAP: target is null");

	IRC_MESSAGE_PARSE("CAP targ");
	assert_eq(ircv3_CAP(s, &m), 1);
	assert_strcmp(line_buf, "CAP: command is null");

	IRC_MESSAGE_PARSE("CAP targ ack");
	assert_eq(ircv3_CAP(s, &m), 1);
	assert_strcmp(line_buf, "CAP: unrecognized subcommand 'ack'");

	IRC_MESSAGE_PARSE("CAP targ xxx");
	assert_eq(ircv3_CAP(s, &m), 1);
	assert_strcmp(line_buf, "CAP: unrecognized subcommand 'xxx'");
}
...
test_ircv3_CAP_LS(void)
{
	...
	assert_strcmp(line_buf, "CAP: parameter is null");
	...
	assert_eq(ircv3_CAP(s, &m), 1);
	assert_eq(send_buf_n, 0);
	assert_strcmp(line_buf, "CAP: invalid parameters");
